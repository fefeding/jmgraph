...
var jmControl = /*#__PURE__*/function (_jmProperty) {
  ...
  _createClass(jmControl, [{
              ...
              case 'transform':
                {
                  if (Array.isArray(styleValue)) {
                    ...
                    _this3.context.transform(styleValue.scaleX, styleValue.skewX, styleValue.skewY, styleValue.scaleY, styleValue.offsetX, styleValue.offsetY);
                  }
    ...
    value: function getLocation() {
      ...
      local.center = this.center && typeof this.center === 'function' ? this.center() : _jmUtils.jmUtils.clone(this.center);
      local.start = this.start && typeof this.start === 'function' ? this.start() : _jmUtils.jmUtils.clone(this.start);
      local.end = this.end && typeof this.end === 'function' ? this.end() : _jmUtils.jmUtils.clone(this.end);
    ...
    value: function checkPoint(p, pad) {
      ...
      if (ps && ps.length) {
        if (rotation && rotation.angle != 0) {
          ps = _jmUtils.jmUtils.clone(ps, true);
    ...
    value: function raiseEvent(name, args) {
      ...
      if (!args.position) {
        ...
        var position = _jmUtils.jmUtils.getEventPosition(args, graph.scaleSize);
      ...
      if (this.is('jmGraph')) {
        ...
        if (this.animateHandles && this.animateHandles.count() > 0) {
          ...
          this.dispatcher = setTimeout(function (_this) {
            ...
            for (var _i = 0; _i < overduehandles.length; _i++) {
